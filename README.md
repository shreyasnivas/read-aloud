...
<img src="Resources/AppIcon-1024.png" width="128" alt="Read Aloud icon">
...
- **Threads**: each thread is a Claude Code session. ⌥⇧A within 15 minutes of
  ...
  `~/Library/Application Support/Read Aloud/` (`threads.json`, plus
  `history/<timestamp>/` with each request's images and answer).
- **Log**: `~/Library/Logs/ReadAloud.log`.
...
`tools/make-signing-cert.sh` once: it creates a local self-signed "Read Aloud
...
Open Read Aloud again from Finder or Spotlight, or pick **Settings…** from
...
- Screen Recording, Microphone and Speech Recognition allowed for Read Aloud.

## Tests

```bash
"build/Read Aloud.app/Contents/MacOS/ReadAloud" --selftest "What's in the red circle?"
open -n "/Applications/Read Aloud.app" --args --transcribe-file /path/to/audio.aiff   # writes audio.aiff.txt
```

## Install without building

Download `Read-Aloud.zip` from the latest release, unzip, and move
`Read Aloud.app` to Applications. It isn't notarized, so the first time,
